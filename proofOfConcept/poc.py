...
def proccesAngle(a):
    ...
    if a == 0:
        ...
        return str(int(math.ceil(abs(a)/90*10)))

    
def processInput(txt):
    if txt[:2] == 'bp':
        cut = txt.split('+')
        print2Log(proccesAngle(float(cut[1].split(',')[2].replace(' ','')))+'\n'+"- "*20,1)
...
whiteSpace.pack()
dataBt = Button(clickFrame, text="Get Data", command= lambda: transmitString('r@'))
dataBt.pack()
